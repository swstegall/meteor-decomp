...
def main() -> int:
    ...
    with out_md.open("w") as f:
        ...
        f.write("\n## Why broader Up-opcode enumeration is HARD\n\n")
        f.write("Three negative findings document why naive byte-pattern scans\n")
        f.write("don't reach a complete Up-opcode list:\n\n")
        f.write("1. **No direct CALL sites for any CPB slot method.** Direct-CALL\n")
        f.write("   scans of the 9 CPB slot fns (3 channels × `Begin` / `BuildHeader`\n")
        f.write("   / `Send` slots 1-3) return ZERO `e8 rel32` direct callers each.\n")
        f.write("   So every Send call goes through vtable indirection.\n\n")
        f.write("2. **Only 22 indirect vtable-slot calls in entire `.text`.** The\n")
        f.write("   `ff <base> <disp8>` pattern (CALL [reg+disp]) appears only\n")
        f.write("   ~22 times across all slot-offsets / base-regs combined. That's\n")
        f.write("   FAR too few to account for all Up traffic in an 11.8 MB `.text`.\n")
        f.write("   MSVC under /O2 prefers the 2-step form `MOV reg, [vtbl+disp];\n")
        f.write("   CALL reg` (`8b 4? disp ff d?`) over the 1-instruction memory-\n")
        f.write("   call form, which is why the static scan undercounts. Detecting\n")
        f.write("   the 2-step form requires light data-flow analysis (track that\n")
        f.write("   the loaded value reaches a CALL register).\n\n")
        f.write("3. **`MOV [reg+0x1c]=imm32` writes don't co-locate with vtable\n")
        f.write("   calls.** Scanning all 175+ register-based `c7 4X 1c <imm32>`\n")
        f.write("   sites for any indirect vtable call within ±96 bytes returns\n")
        f.write("   ZERO matches. The opcode write and the Send call are\n")
        f.write("   separated by enough intervening code that a windowed scan\n")
        f.write("   misses them — OR the Up-send path doesn't go through the\n")
        f.write("   CPB.Send vtable slot at all (more likely: senders enqueue\n")
        f.write("   directly to the IpcChannel, with the CPB just acting as a\n")
        f.write("   header-builder facade).\n\n")
        f.write("Producing a complete Up-opcode enumeration requires Ghidra-driven\n")
        f.write("constant propagation through the CPB pointer load + opcode write\n")
        f.write("+ following each CPB instance's lifetime to its Send call. That's\n")
        f.write("a symbolic-execution task, not a static byte-pattern scan. The\n")
        f.write("9 opcodes recovered here via direct-ctor-CALL analysis are the\n")
        f.write("subset reachable WITHOUT symbolic execution.\n\n")
        f.write("## What's also NOT in this report\n\n")
