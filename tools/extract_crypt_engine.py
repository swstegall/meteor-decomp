...
def main() -> int:
    ...
    lines.append("")
    lines.append("1. **Lobby packets are routinely *over*-encrypted by the server**, ")
    lines.append("   then partly mis-decrypted by the client — and this is **benign**: ")
    lines.append("   slots 6/7 round body length DOWN to a multiple of 32, leaving ")
    lines.append("   the trailing 0–31 bytes ciphertext on the wire. Both Project ")
    lines.append("   Meteor's reference C# server and garlemald-server encrypt the ")
    lines.append("   full `subpacketSize - 16` body (8-aligned, not 32-aligned). The ")
    lines.append("   protocol's lobby buffers are over-provisioned (`MemoryStream(0x98)`, ")
    lines.append("   `(0x210)`, `(0x280)`, etc.) so the meaningful content fills only ")
    lines.append("   the prefix and the trailing region is zero padding the client ")
    lines.append("   never reads — the mis-decrypted bytes always fall inside that ")
    lines.append("   padding. Project Meteor has shipped against the real 1.x client ")
    lines.append("   under this discrepancy for years. See ")
    lines.append("   `include/net/lobby_proto_channel.h` for worked examples ")
    lines.append("   (`SelectCharacterConfirm` 0x98, `AccountList` 0x280).")
