...
The table below is regenerated by `make update-docs` from
`tools/progress.py --json` + `docs/reconcile-state.json` (`make reconcile`).
The durable, authoritative metric is the committed `_rosetta/*.cpp` solved
