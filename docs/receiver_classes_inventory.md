...
| #8c | Look for pre-kick "receiver state init" packets that prime `context_root[+0x128]` (would shift Branch B1 → Branch B2) | 🟡 advanced 2026-05-17 — `docs/context_root_128_priming_hunt.md`. **`context_root` class definitively identified as `Application::Lua::Script::Client::Control::NpcBase`** (vtable `0xfd647c`, 41 slots). Writer set narrowed from 98 → 57 → 26 candidates (4 ctors/clearers + ~10 bulk-copy + ~12 misc); **NO RECEIVER 1-hop-directly calls a writer** — priming is multi-level or via vtable dispatch. Most likely indirect path: `SetEventStatusReceiver` → `FUN_006e67c0` → vector_find in npc's 4 inline 16-byte handler slots at `+0xe8/+0xf8/+0x108/+0x118` (which END immediately before `+0x128`) → handler.vtable[9]. Definitive close needs handler-class identification (static) or runtime HWBP (Wine debugger). |
