...
| #8c | Look for pre-kick "receiver state init" packets that prime `context_root[+0x128]` (would shift Branch B1 → Branch B2) | 🟡 advanced 2026-05-17 (deep) — `docs/context_root_128_priming_hunt.md`. **`context_root` class definitively identified as `Application::Lua::Script::Client::Control::NpcBase`** (vtable `0xfd647c`, 41 slots). Writer-set narrowed 98 → 57 → 26. **Identified Push event-condition handler classes** (104 B `0x1056edc` / 100 B `0x1056f10`) installed by `SetPushEventCondition*` receivers via `FUN_006f2310`; their **vtable[9] = `FUN_00894d30`** (same for both) doesn't write +0x128 directly, nor do its 7 sub-helpers — Push triggers aren't the primer. **Final architectural conclusion**: ALL static writers to `npc[+0x128]` in the Lua-actor range write NO_ACTOR (FUN_006e32f0 the sharp clearer + FUN_00703970 selective despawn-clearer + FUN_00706700 state-machine clearer). **NO static C++ path writes a real actor id to +0x128**. The priming MUST happen via (a) Lua-engine binding called from shipped scripts, (b) bulk-copy/load-state (FUN_008f0a70), or (c) computed vtable dispatch. Next steps: grep .lpb corpus for Lua-bound NpcBase setters, walk FUN_008f0a70 callers, or runtime HWBP. |
