...
//
// FUNCTION: ffxivgame 0x00409e60 — singleton get-or-set accessor with
// SEH-protected lazy initialization (__cdecl, 104 B).
//
// Byte-for-byte quadruplet of FUN_00409260 / FUN_00409840 / FUN_00409b50
// (same EH3-style SEH frame, same init helper CALL target at RVA
// 0x0040e500, same get-or-set shape) — only the scope_table address and
// the two .data globals differ.
//
// Semantics (recovered from asm):
//
//   void *FUN_00409e60(void *replacement) {
...
//
// Two globals (in .data):
//   0x01327bb0  g_flag       — initialized-once flag (bit 0 is "init done")
//   0x01327bac  g_instance   — singleton pointer (set lazily, optionally replaced)
//
// The init CALL targets RVA 0x0040e500 (rel32 from this site) — the
// same helper called by FUN_00409260, FUN_00409840, and FUN_00409b50.
//
// Original 104 bytes (per asm/ffxivgame/00009e60_FUN_00409e60.s):
//
//   00009e60: 64 a1 00 00 00 00 6a ff 68 de 4b e5 00 50 b8 01
//   00009e70: 00 00 00 64 89 25 00 00 00 00 84 05 b0 7b 32 01
//   00009e80: 75 18 09 05 b0 7b 32 01 c7 44 24 08 00 00 00 00
//   00009e90: e8 6b 46 00 00 a3 ac 7b 32 01 8b 44 24 10 85 c0
//   00009ea0: 74 13 a3 ac 7b 32 01 8b 0c 24 64 89 0d 00 00 00
//   00009eb0: 00 83 c4 0c c3 8b 0c 24 a1 ac 7b 32 01 64 89 0d
//   00009ec0: 00 00 00 00 83 c4 0c c3
//
...
extern "C" __declspec(naked) void FUN_00409e60() {
    ...
    get_path:
        mov     ecx, [esp]                                  // 8b 0c 24        (3 B)
        mov     eax, g_singleton_instance_00409e60          // a1 ?? ?? ?? ??  (5 B, reloc +1)
        mov     fs:[0], ecx                                 // 64 89 0d 00 00 00 00 (7 B)
        add     esp, 0xc                                    // 83 c4 0c        (3 B)
        ret                                                 // c3              (1 B)
    }
