...
//
// FUNCTION: ffxivgame 0x0044c690 — `__thiscall` vtable-stamp + tail-handoff
//                                   (18 B / 0x12).
//
// Behaviour read from the disassembly at orig RVA 0x0004c690:
//
...
//   c7 06 6c 73 f6 00  mov  dword [esi], 0xf6736c  ; this->vftable = 0x00f6736c
//   e8 47 55 58 00     call 0x009d1be9             ; hand off (no epilogue)
//
//   __thiscall: ECX carries `this`. The body re-stamps the object's
//   vtable pointer to 0x00f6736c (a .rdata vftable, reloc-bearing in the
...
//
//   This is the exact twin of sibling FUN_0044c6d0; only the vftable
//   immediate (0xf6736c vs 0xf6737c) and the CALL rel32 differ.
//
...
//   +0x09   absolute vftable imm32   .rdata 0x00f6736c
//   +0x0e   rel32 CALL               .text  0x009d1be9 (rel 0x00585547)
//
...
//   adjacent. The pragmatic choice — identical to siblings FUN_0044c6d0
//   and FUN_00401730 — is a `__declspec(naked)` body re-emitting the orig
//   18 bytes verbatim via MASM `_emit` directives. The .obj's `.text` ends
//   up byte-identical to the orig slice (raw immediates, no relocations),
//   which is what tools/compare.py grades against.

extern "C" __declspec(naked) void FUN_0044c690() {
    __asm {
        _emit 0x56
