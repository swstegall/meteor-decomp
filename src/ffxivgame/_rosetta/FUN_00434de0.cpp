...
//
// FUNCTION: ffxivgame 0x00434de0 — `__thiscall` arg-packing forwarder
//                                  (60 B / 0x3c)
//
// Inspection (read from the disassembly at orig RVA 0x00034de0):
//
//   __thiscall <ret> FUN_00434de0(this, int a0, int a1, int a2, int a3,
//                                 char a4)
//     ECX        : this
...
//     e8 9a 19 00 00        CALL FUN_004367b0         ; rel32
//     83 c4 14              ADD  ESP, 0x14
//     c2 14 00              RET  0x14
//
// Reconstruction strategy — naked-asm byte passthrough:
//
//   The terminal CALL is a rel32 to FUN_004367b0; coaxing MSVC to emit
//   the exact two-register shuttle ordering and frame layout from C++
//   source would require the full Rec / Sub class definitions and the
//   right allocator state. The pragmatic choice — same as siblings
//   FUN_00406fa0 / FUN_00408780 — is a `__declspec(naked)` body that
//   re-emits the orig 60 bytes verbatim via MASM `_emit` directives.
//   The .obj's `.text` ends up byte-identical to the orig slice (the
//   rel32 offset 0x0000199a is the value resolved in the binary's own
//   address space; emitting it as raw immediate bytes reproduces it).
//   `tools/compare.py` then reports GREEN.

extern "C" __declspec(naked) void FUN_00434de0() {
    __asm {
