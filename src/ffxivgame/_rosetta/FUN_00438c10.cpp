...
//
// FUNCTION: ffxivgame 0x00438c10 — __thiscall wrapper that stages a 3-DWORD
//   local struct on the stack and forwards it to the virtual-call gate
//   FUN_00435ef0 (46 B / 0x2e).
//
...
//     [ESP+0x04] : DWORD  arg1   (placed directly in local[1])
...
//   ECX = &local_struct   (LEA ECX, [ESP+0x4] after PUSH EAX)
...
// Frame: SUB ESP, 0xc (12-byte local) + PUSH EAX (arg staging).
//
// Differs from sibling FUN_00438be0 in that neither stack arg is
// dereferenced through a pointer here — both arg1 and arg2 are stored
// into the local struct by value directly.
//
// Asm (46 bytes @ orig RVA 0x00038c10):
//   83 ec 0c                    SUB  ESP, 0xc
//   8b 44 24 10                 MOV  EAX, [ESP+0x10]       ; arg1
//   8b 54 24 14                 MOV  EDX, [ESP+0x14]       ; arg2
//   89 44 24 04                 MOV  [ESP+0x4], EAX        ; stash arg1
//   8b 41 04                    MOV  EAX, [ECX+0x4]        ; this->field4
//   50                          PUSH EAX                   ; stage this->field4
//   8d 4c 24 04                 LEA  ECX, [ESP+0x4]        ; ECX = &local
//   c7 44 24 04 d0 49 f6 00     MOV  [ESP+0x4], 0xf649d0   ; local[0] = type tag
//   89 54 24 0c                 MOV  [ESP+0xc], EDX        ; local[2] = arg2
//   e8 b8 d2 ff ff              CALL FUN_00435ef0           (REL32 — reloc)
...
// (arg1 stashed at [ESP+0x4] before the PUSH shifts the frame) and the
...
extern "C" __declspec(naked) void FUN_00438c10() {
    __asm {
        sub     esp, 0xc
