...
//
// FUNCTION: ffxivgame 0x0001b780 — MSVC scalar deleting destructor,
//                                   vtable-set + thiscall-free form (36 B)
//
// Member of the 36-byte scalar-deleting-dtor family (same shape as
// FUN_00419e40 / FUN_0041ab70):
//   - resets vtable at [this+0] to 0x00f58208 before delegating to the
//     real destructor body (FUN_0041a9b0, a __thiscall with SEH frame)
...
// stack arg (RET 4).
//
// Asm (36 bytes @ orig RVA 0x0001b780):
//   56                    PUSH ESI
//   8b f1                 MOV ESI, ECX                ; this
//   c7 06 RR RR RR RR     MOV dword ptr [ESI], vtable ; set vtable (reloc)
//   e8 RR RR RR RR        CALL FUN_0041a9b0            ; real dtor (__thiscall)
...
extern "C" void FUN_0041a9b0();      // real destructor (__thiscall, no stack args)
extern "C" void FUN_0040df70();      // thiscall free, one stack arg (callee RET 4)
extern "C" int g_vtable_1b780;       // vtable data at VA 0x00f58208

extern "C" __declspec(naked) void FUN_0041b780(unsigned int) {
    __asm {
        push esi
        mov esi, ecx
        mov dword ptr [esi], offset g_vtable_1b780
        call FUN_0041a9b0
