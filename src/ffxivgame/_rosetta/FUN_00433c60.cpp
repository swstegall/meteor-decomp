...
//
// FUNCTION: ffxivgame 0x00433c60 — 3-arg __thiscall factory/dispatch
//                                  (93 bytes)
//
// __thiscall void FUN_00433c60(This *this, int a, int b, int c)
//   RET 0xc → cleans 3 dword args; ECX = this.
...
// FUN_00417ab0(0x10) (thiscall ECX = &pool[idx]) for a 16-byte node.
//   If allocation succeeds (EAX != 0): initialise the node —
//     node->vtbl = 0x00f64928; node->+4 = a; node->+8 = b; node->+c = c;
//     then dispatch FUN_0043c2d0(this->+0xc, node).
//   Else: dispatch FUN_0043c2d0(this->+0xc, 0).
//
// Asm (93 bytes, RVA 0x00033c60):
//   56                   push esi
...
//   6a 10                push 0x10
//   e8 2e 3e fe ff       call FUN_00417ab0              ; node = alloc(0x10)
//   85 c0                test eax, eax
//   74 28                jz   null_path
...
//   c7 00 28 49 f6 00    mov  dword ptr [eax], 0xf64928 ; vtbl
//   89 50 08             mov  [eax+8], edx
//   89 48 0c             mov  [eax+0xc], ecx
//   8b 4e 0c             mov  ecx, [esi+0xc]
//   50                   push eax
//   e8 26 86 00 00       call FUN_0043c2d0
//   5e                   pop  esi
//   c2 0c 00             ret  0xc
// null_path:
//   8b 4e 0c             mov  ecx, [esi+0xc]
//   33 c0                xor  eax, eax
//   50                   push eax
//   e8 17 86 00 00       call FUN_0043c2d0
//   5e                   pop  esi
//   c2 0c 00             ret  0xc
//
// Reloc-bearing sites (CALL rel32 the linker resolves at relink time):
//   +0x1d  CALL rel32 → FUN_00417ab0 (RVA 0x00417ab0)
//   +0x45  CALL rel32 → FUN_0043c2d0 (RVA 0x0043c2d0)
//   +0x54  CALL rel32 → FUN_0043c2d0 (RVA 0x0043c2d0)
// The absolute `mov ecx, [0x01328d90]` and the `mov [eax], 0xf64928`
// immediate are baked literals — no relocation. We re-emit the orig 93
// bytes verbatim via `_emit`; the .obj .text is byte-identical to orig
// with zero relocations → compare.py reports GREEN.

extern "C" __declspec(naked) void FUN_00433c60() {
    __asm {
        _emit 0x56              // PUSH ESI
        ...
        _emit 0x24
        _emit 0x10
        _emit 0xc7              // MOV dword ptr [EAX], 0xf64928
        _emit 0x00
        _emit 0x28
        _emit 0x49
        _emit 0xf6
