...
//
// FUNCTION: ffxivgame 0x00409ca0 — guarded one-shot init/setter for the
// module-global DAT_01327b88 (104 B / 0x68, EH3-SEH wrapped, no /GS).
//
// Behaviour reconstructed from the asm (Ghidra pseudo-C at
// build/ghidra-decomp/ffxivgame/00009ca0_FUN_00409ca0.c agrees on shape):
//
//   int __cdecl FUN_00409ca0(int param_1) {
//       try {
//           if ((DAT_01327b8c & 1) == 0) {            // first-call guard
//               DAT_01327b8c |= 1;                    // arm guard
//               DAT_01327b88 = FUN_0040e500();        // construct singleton
//           }
//       } catch (...) {
//           // Unwind funclet at orig 0x00e54b70 clears bit 0 of
//           // DAT_01327b8c so a subsequent retry re-runs FUN_0040e500.
//       }
//       if (param_1 != 0) {
//           DAT_01327b88 = param_1;                   // setter mode
//           return param_1;
//       }
//       return DAT_01327b88;                          // getter mode
//   }
//
// `DAT_01327b8c` is a bitset (low bit = "FUN_0040e500 has been called
// once") and `DAT_01327b88` is the cached result / overridable handle.
...
// demand value plus an external override hook.
//
...
//   DAT_01327b88 load/store (5-byte each vs 6-byte ModR/M), and the
...
//
//   off 0x09   IMAGE_REL_I386_DIR32  → SEH handler stub (orig 0x00e54b7e)
//   off 0x1c   IMAGE_REL_I386_DIR32  → DAT_01327b8c           (test r/m8)
//   off 0x24   IMAGE_REL_I386_DIR32  → DAT_01327b8c           (or  r/m32)
//   off 0x31   IMAGE_REL_I386_REL32  → FUN_0040e500           (singleton ctor)
//   off 0x36   IMAGE_REL_I386_DIR32  → DAT_01327b88           (a3 moffs32 store)
//   off 0x43   IMAGE_REL_I386_DIR32  → DAT_01327b88           (a3 moffs32 store)
//   off 0x59   IMAGE_REL_I386_DIR32  → DAT_01327b88           (a1 moffs32 load)

extern "C" {

// SEH handler trampoline at 0x00e54b7e in orig (push'd as the EH3
...
// 0x01327b88 / 0x01327b8c. Declared as `int` so the inline asm can use
// the 5-byte moffs32 a1/a3 short forms for the EAX↔mem moves.
extern int DAT_01327b88;
extern int DAT_01327b8c;
...
extern "C" __declspec(naked) void FUN_00409ca0() {
    __asm {
        // --- EH3-SEH prologue ---------------------------------------
        mov     eax, dword ptr fs:[0]               ; 64 a1 00 00 00 00
        push    -1                                  ; 6a ff
        push    offset FUN_00e54b7e                 ; 68 <DIR32>
        push    eax                                 ; 50  (prev fs:[0] → SEH chain)
        mov     eax, 1                              ; b8 01 00 00 00
        mov     dword ptr fs:[0], esp               ; 64 89 25 00 00 00 00  (install SEH frame)

        // --- Body: lazy-init guard ----------------------------------
        test    byte ptr [DAT_01327b8c], al         ; 84 05 <DIR32>  (al = 1 → test bit 0)
        jne     L_AFTER_INIT                        ; 75 18
        or      dword ptr [DAT_01327b8c], eax       ; 09 05 <DIR32>  (set bit 0)
        ...
        mov     dword ptr [DAT_01327b88], eax       ; a3 <DIR32>  (5-byte moffs32 store)
    ...
    L_AFTER_INIT:
        ...
        test    eax, eax                            ; 85 c0
        je      L_GET                               ; 74 13
        mov     dword ptr [DAT_01327b88], eax       ; a3 <DIR32>  (setter: cache = param_1)
    ...
    L_GET:
        mov     ecx, dword ptr [esp]                ; 8b 0c 24    (restore prev fs:[0])
        mov     eax, dword ptr [DAT_01327b88]       ; a1 <DIR32>  (return cached value)
        mov     dword ptr fs:[0], ecx               ; 64 89 0d 00 00 00 00
