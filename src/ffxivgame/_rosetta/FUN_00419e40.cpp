...
//
// FUNCTION: ffxivgame 0x00019e40 — MSVC scalar deleting destructor,
//                                   vtable-set + thiscall-free form (36 B)
//
// Variant of the 36-byte scalar-deleting-dtor family:
//   - resets vtable at [this+0] before delegating to the real destructor
//     (same pattern as FUN_00417610 / FUN_004167e0)
//   - frees memory via __thiscall FUN_0040df70 with this = *(this-4), i.e.
//     the allocator pointer is stored one slot before the object (unlike the
//     cdecl + add-esp-4 form used in the above siblings)
//
...
// stack arg (RET 4).
//
// Asm (36 bytes @ orig RVA 0x00019e40):
//   56                    PUSH ESI
//   8b f1                 MOV ESI, ECX                ; this
//   c7 06 RR RR RR RR     MOV dword ptr [ESI], vtable ; set vtable (reloc)
//   e8 RR RR RR RR        CALL FUN_00430890            ; real dtor (__thiscall)
...
extern "C" void FUN_00430890();      // real destructor (__thiscall, no stack args)
extern "C" void FUN_0040df70();      // thiscall free, one stack arg (callee RET 4)
extern "C" int g_vtable_19e40;       // vtable data at VA 0x0113c378

extern "C" __declspec(naked) void FUN_00419e40(unsigned int) {
    __asm {
        push esi
        mov esi, ecx
        mov dword ptr [esi], offset g_vtable_19e40
        call FUN_00430890
