...
//
// FUNCTION: ffxivgame 0x0040a100 — singleton get-or-set accessor with
// SEH-protected lazy initialization (__cdecl, 104 B).
//
// Byte-for-byte quintuplet of FUN_00409260 / FUN_00409840 / FUN_00409b50 /
// FUN_00409e60 (same EH3-style SEH frame, same init helper CALL target at
// RVA 0x0040e500, same get-or-set shape) — only the scope_table address
// and the two .data globals differ.
//
// Semantics (recovered from asm):
//
//   void *FUN_0040a100(void *replacement) {
...
//
// Two globals (in .data):
//   0x01327be8  g_flag       — initialized-once flag (bit 0 is "init done")
//   0x01327be4  g_instance   — singleton pointer (set lazily, optionally replaced)
//
// The init CALL targets RVA 0x0040e500 (rel32 from this site) — the
// same helper called by the four siblings above.
//
// Original 104 bytes (per asm/ffxivgame/0000a100_FUN_0040a100.s):
//
//   0000a100: 64 a1 00 00 00 00 6a ff 68 7e 4c e5 00 50 b8 01
//   0000a110: 00 00 00 64 89 25 00 00 00 00 84 05 e8 7b 32 01
//   0000a120: 75 18 09 05 e8 7b 32 01 c7 44 24 08 00 00 00 00
//   0000a130: e8 cb 43 00 00 a3 e4 7b 32 01 8b 44 24 10 85 c0
//   0000a140: 74 13 a3 e4 7b 32 01 8b 0c 24 64 89 0d 00 00 00
//   0000a150: 00 83 c4 0c c3 8b 0c 24 a1 e4 7b 32 01 64 89 0d
//   0000a160: 00 00 00 00 83 c4 0c c3
//
...
extern "C" __declspec(naked) void FUN_0040a100() {
    ...
    get_path:
        mov     ecx, [esp]                                  // 8b 0c 24        (3 B)
        mov     eax, g_singleton_instance_0040a100          // a1 ?? ?? ?? ??  (5 B, reloc +1)
        mov     fs:[0], ecx                                 // 64 89 0d 00 00 00 00 (7 B)
        add     esp, 0xc                                    // 83 c4 0c        (3 B)
        ret                                                 // c3              (1 B)
    }
