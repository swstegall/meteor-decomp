...
//
// FUNCTION: ffxivgame 0x00421d40 — linked-list range-extract (200 B / 0xc8)
//                                  Structural twin of FUN_00421c70; identical
//                                  byte layout with different intra-binary CALL
//                                  targets.  Same __thiscall + 5-stack-arg
//                                  (RET 0x14) signature, same three
//                                  _SECURE_SCL-style iterator-validity asserts,
//                                  same happy-path bulk-clear + sentinel reset,
//                                  same loop calling an advance helper then an
//                                  insert helper until first == last.
//
// Signature (recovered from asm):
//
//   __thiscall void FUN_00421d40(
//       void*    this    @ ECX,
...
//   [ESP+0x08]  saved EBP
//   [ESP+0x0c]  saved EBX
//   [ESP+0x10]  local1  (8-byte SUB-ESP,8 area; used as out-buf for FUN_004218e0)
//   [ESP+0x14]  local2
...
//   +0x4a  CALL 0x00c2bb90  (__thiscall bulk-transfer helper, 1 arg)
//   +0x99  CALL 0x009172c0  (__thiscall advance helper, no extra args)
//   +0xa7  CALL 0x004218e0  (__thiscall insert helper, 3 args, RET 0xC)
//
...
//
// Reconstruction: naked-asm symbolic passthrough — identical strategy as
// FUN_00421c70 (the only differences are the three intra-binary CALL targets).
// All five CALLs emit COFF rel32 relocations that compare.py masks; all
// remaining bytes are deterministic MSVC 2005 x86 encodings.  Short-jump
// form is preserved throughout (all targets within ±127 bytes).
// The trailing 0x14 00 of the final RET 0x14 straddles the 200-byte
// boundary: only the first two bytes (c2 14) fall inside the window; the
// 00 byte lives at the exclusive end (0x21e08).

extern "C" {
    void FUN_009d22b4();   // assertion / _SECURE_SCL check
    void FUN_00c2bb90();   // bulk-transfer __thiscall helper (1 arg, RET 4)
    void FUN_009172c0();   // advance __thiscall helper (no stack args, RET)
    void FUN_004218e0();   // insert __thiscall helper (3 stack args, RET 0xC)
}

extern "C" __declspec(naked) void FUN_00421d40() {
    __asm {
        sub  esp, 8
        push ebx
        push ebp
    ...
    l_skip3:
        ...
        call FUN_009172c0                   // advance arg2 in place
        push ebx                            // arg3
        push edi                            // arg2
        lea  edx, [esp + 0x18]             // &local1 (output slot for FUN_004218e0)
        push edx
        mov  ecx, esi                       // ECX = this
        call FUN_004218e0                   // this->insert(&local, arg2, arg3); RET 0xC
