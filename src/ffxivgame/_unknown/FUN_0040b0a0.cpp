...
//
// FUNCTION: ffxivgame 0x0000b0a0 — assertion-failure handler
//                                   (__cdecl, 166 B / 0xa6)
//
// void FUN_0040b0a0(const char *cond, const char *msg,
...
// No /GS cookie (no array >= 5 bytes condition — but the 2048-byte buffer
// would normally trigger /GS; the orig binary does NOT emit a cookie check
// here, which means either /GS was suppressed for this TU or the compiler
// classified this as a leaf / trusted function).
//
...
//     two CALL rel32 → _snprintf_s (RVA 0x009d4f9f — 0x9e4fa4 from site)
...
// Using __declspec(dllimport) would cause FF 15 [__imp_fn] form,
// but we need FF 15 [0x012651b4] — use a direct function pointer decl.
typedef void (__cdecl *OutputFn)(const char *buf, int level);
extern OutputFn g_outputFnSlot; // slot at 0x012651b4 holds the fn ptr

// Format strings in the orig binary's .rdata.
extern const char g_fmtThread[];   // "%s(%d):[%s] <assert> (%s) %s\n"
extern const char g_fmtNoThread[]; // "%s(%d): <assert> (%s) %s\n"

// The intentional null-deref crash target.
extern volatile int g_crashTarget; // at absolute address 0x00000000
