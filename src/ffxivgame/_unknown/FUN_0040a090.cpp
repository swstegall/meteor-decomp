...
//
// FUNCTION: ffxivgame 0x0000a090 — magic-static get-or-set accessor for a
//                                  globally-cached pointer, wrapped in an
//                                  EH3 (`__except_handler3`-style) SEH
//                                  frame (104 B / 0x68).
//
// Inspection (read from the disassembly at orig RVA 0x0000a090):
//
//   __cdecl void *FUN_0040a090(void *newValue);
//
//     static bool s_initialised = false;        // .data 0x01327be0 (low bit)
//     static void *s_inst = nullptr;            // .data 0x01327bdc
//
//     if (!s_initialised) {
//         s_initialised = true;
...
//
//   The function is the classic Win32 EH3 wrap around a guarded
//   magic-static initialisation followed by a get-or-set on the
//   resulting global, which keeps both the lazy-init `if (!flag)` and
//   the setter `if (arg)` arms inside the same SEH scope (so a throwing
//   `FUN_0040e500` would unwind through the registered handler).
//
//   Stack frame (after the EH3 prologue, ESP-relative):
//     [esp+0x00]   saved FS:[0] chain link
//     [esp+0x04]   EH3 scope-table address (0x00e54c5e)
...
//     +0x08   scope-table handler RVA    (0x00e54c5e — .rdata FuncInfo)
//     +0x13   FS:[0] install             (constant 0, fold-through)
//     +0x1a   init-flag TEST byte        (.data 0x01327be0)
//     +0x22   init-flag OR  byte         (.data 0x01327be0)
//     +0x30   lazy-init CALL             (.text 0x0040e500 rel32 — __cdecl)
//     +0x35   s_inst store               (.data 0x01327bdc)
//     +0x42   s_inst store (setter)      (.data 0x01327bdc, 2nd)
//     +0x4a   FS:[0] restore (setter)    (constant 0, fold-through)
//     +0x58   s_inst load  (getter)      (.data 0x01327bdc, 3rd)
...
//   under /O2 — the cookie-vs-no-cookie distinction in particular
//   forces /EHs (synchronous EH only, no `noexcept`) plus a
//   `__declspec(nothrow)` annotation on `FUN_0040e500` to talk MSVC
//   out of emitting the EH4 cookie. Even then state-numbering and
//   short-vs-near branching tend to drift by a byte or two.
//
//   The pragmatic choice — the same one FUN_00401650 took for its
//   EH4-SEH-wrapped singleton init, FUN_00401a00 took for its
//   SEH-wrapped exe-dir bootstrap, and FUN_004014b0 took for its
//   SEH-wrapped tick fn — is a `__declspec(naked)` body that re-emits
...
//   (FUN_0040e500) and the global being cached at .data 0x01327bdc
//   are catalogued under decomp-notes/types/.

extern "C" __declspec(naked) void FUN_0040a090() {
    __asm {
        ...
        _emit 0xff
        _emit 0x68              // PUSH 0xe54c5e            (scope-table)
        _emit 0x5e
        _emit 0x4c
        _emit 0xe5
        _emit 0x00
        ...
        _emit 0x00
        _emit 0x00
        _emit 0x84              // TEST [0x01327be0], AL    (init flag)
        _emit 0x05
        _emit 0xe0
        _emit 0x7b
        _emit 0x32
        ...
        _emit 0x18
        _emit 0x09              // OR   [0x01327be0], EAX   (set init flag)
        _emit 0x05
        _emit 0xe0
        _emit 0x7b
        _emit 0x32
        ...
        _emit 0x00
        _emit 0x00
        _emit 0xa3              // MOV  [0x01327bdc], EAX   (s_inst = init result)
        _emit 0xdc
        _emit 0x7b
        _emit 0x32
        _emit 0x01
        ...
        _emit 0x13
        _emit 0xa3              // MOV  [0x01327bdc], EAX   (setter: s_inst = arg)
        _emit 0xdc
        _emit 0x7b
        _emit 0x32
        _emit 0x01
        ...
        _emit 0x0c
        _emit 0x24
        _emit 0xa1              // MOV  EAX, [0x01327bdc]   (getter: return s_inst)
        _emit 0xdc
        _emit 0x7b
        _emit 0x32
        _emit 0x01
